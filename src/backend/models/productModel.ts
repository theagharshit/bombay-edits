...
export class ProductModel {
  ...
  public static findBySlug(slug: string): Product | undefined {
  ...
  public static findByCollection(collection: string): Product[] {
    return getProductsByCollection(collection);
  }

  public static findByOccasion(occasion: string): Product[] {
    return getProductsByOccasion(occasion);
  }

  public static getNewArrivals(): Product[] {
  ...
  public static query(options: ProductQueryOptions = {}): {
    products: Product[];
    total: number;
    page: number;
    limit: number;
    ...
    const limit = Math.max(1, options.limit || total || 100);
  ...
  public static async queryAsync(options: ProductQueryOptions = {}): Promise<{
    products: Product[];
    total: number;
    page: number;
    limit: number;
  ...
  public static checkStock(
    ...
    if (!product) {
      return { inStock: false, availableQuantity: 0, isMadeToOrder: false };
    }

    if (product.isMadeToOrder) {
      return { inStock: true, availableQuantity: 99, isMadeToOrder: true };
    }

    const stock = (product.stockBySize as Record<string, number>)?.[size] ?? 0;
    return {
