...
const FIELDS = [
  ...
  { key: 'store_email', label: 'Contact Email', type: 'email', placeholder: 'hello@thebombayedit.com' },
  { key: 'store_phone', label: 'Phone Number', type: 'text', placeholder: '+977 ...' },
  { key: 'store_address', label: 'Store Address', type: 'textarea', placeholder: 'Full address...' },
  { key: 'store_city', label: 'City', type: 'text', placeholder: 'Kathmandu' },
  { key: 'store_country', label: 'Country', type: 'text', placeholder: 'Nepal' },
  { key: 'store_instagram', label: 'Instagram Handle', type: 'text', placeholder: '@thebombayedit' },
  { key: 'store_whatsapp', label: 'WhatsApp Number', type: 'text', placeholder: '+977 ...' },
  { key: 'seo_default_title', label: 'Default SEO Title', type: 'text', placeholder: 'The Bombay Edit | Luxury Indian Wear' },
  { key: 'seo_default_description', label: 'Default Meta Description', type: 'textarea', placeholder: 'Discover...' },
];

export function StoreSettingsClient({ settings }: { settings: Record<string, any> }) {
      ...
      <div className="bg-white border rounded shadow-sm divide-y">
        {FIELDS.map((field) => (
          <div key={field.key} className="p-5 flex flex-col md:flex-row md:items-start gap-4">
            <label className="text-sm font-medium text-gray-700 md:w-48 flex-shrink-0 pt-1">{field.label}</label>
            {field.type === 'textarea' ? (
              <textarea
