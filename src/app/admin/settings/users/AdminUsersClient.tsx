    ...
    <div className="p-4 md:p-8 max-w-4xl mx-auto pb-32">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Manage who can access the admin panel. User creation is done via the CLI or database for
          security.
        </p>
      </div>
              ...
              users.map((u) => (
                <tr key={u.id} className="hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 hidden md:table-cell">
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider ${ROLE_COLORS[u.role] || 'bg-gray-100 text-gray-600'}`}
                    >
                      {u.role}
                    </span>
                  </td>
      ...
      <div className="mt-6 p-4 bg-amber-50 border border-amber-200 rounded text-sm text-amber-800">
        <strong>Note:</strong> Admin user creation and role changes are restricted to database-level
        operations for security. Contact a developer to add or modify admin accounts.
      </div>
    </div>
  );
}
