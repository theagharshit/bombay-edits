...
export function NavigationClient({ items }: { items: NavItem[] }) {
  ...
  const handleToggle = async (id: string, isActive: boolean) => {
    try {
      await toggleNavItem(id, isActive);
      router.refresh();
    } catch (e: unknown) {
      toast.error(e instanceof Error ? e.message : 'Error');
    }
  ...
  const NavRow = ({ item, indent = 0 }: { item: NavItem; indent?: number }) => (
    <>
      <tr className="hover:bg-gray-50">
        ...
        <td className="py-3 px-4 text-gray-500 text-xs font-mono hidden md:table-cell">
          {item.href}
        </td>
      ...
      {children(item.id).map((child) => (
        <NavRow key={child.id} item={child} indent={indent + 1} />
      ))}
    </>
  );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                required
                value={editing.href}
                onChange={(e) => setEditing((f) => (f ? { ...f, href: e.target.value } : f))}
            ...
            <div>
              ...
              <input
                value={editing.group}
                onChange={(e) => setEditing((f) => (f ? { ...f, group: e.target.value } : f))}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditing((f) => (f ? { ...f, sortOrder: parseInt(e.target.value) } : f))
                }
                className="w-full px-3 py-2 border rounded text-sm"
              />
            </div>
            <div>
              ...
              <select
                ...
                onChange={(e) => setEditing((f) => (f ? { ...f, parentId: e.target.value } : f))}
            ...
            <div className="flex items-center gap-2 pt-5">
              <input
                ...
                onChange={(e) => setEditing((f) => (f ? { ...f, isActive: e.target.checked } : f))}
                className="rounded"
              />
