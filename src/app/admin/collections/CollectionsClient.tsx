...
export function CollectionsClient({ collections }: Props) {
  ...
  const handleAddProduct = (product: CollectionMember['product']) => {
    ...
    startTransition(async () => {
      ...
      setMembers(prev => prev.some(m => m.productId === product.id) ? prev : [...prev, { productId: product.id, product }]);
      setProductSearch('');
      setSearchResults([]);
    });
  };
    ...
    <div>
      ...
      {collections.length === 0 ? (
        <EmptyState
          ...
          action={<button onClick={() => openEditor('new')} className="px-3 py-2 text-sm font-medium rounded-[var(--admin-radius)] bg-[var(--admin-accent)] text-white">Add Collection</button>}
        />
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--admin-border)] bg-[var(--admin-surface)]">
                ...
                <th className="px-3 py-2 text-left text-xs font-medium text-[var(--admin-text-mute)]">Slug</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-[var(--admin-text-mute)]">Products</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-[var(--admin-text-mute)]">Featured</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-[var(--admin-text-mute)]">Status</th>
                <th className="px-3 py-2 w-24" />
              </tr>
            </thead>
            <tbody>
              {collections.map(col => (
                <tr key={col.id} className="border-b border-[var(--admin-border)] last:border-0 hover:bg-[var(--admin-surface)] h-[var(--admin-row-h)]">
                  <td className="px-3">
                    {col.image
                      ? <img src={col.image} alt="" className="w-8 h-8 rounded object-cover" />
                  ...
                  <td className="px-3 font-mono text-xs text-[var(--admin-text-mute)]">{col.slug}</td>
                  <td className="px-3">
                    <a href={`/admin/products?collectionId=${col.id}`} className="flex items-center gap-1 text-xs text-[var(--admin-info)] hover:underline">
                  ...
                  <td className="px-3">
                    ...
                    >
                      <Star className={`w-4 h-4 ${col.isFeatured ? 'fill-[var(--admin-warn)] text-[var(--admin-warn)]' : 'text-[var(--admin-border-str)]'}`} />
                    </button>
                  </td>
                  <td className="px-3">
                    <StatusBadge status={col.isActive ? 'ACTIVE' : 'ARCHIVED'} label={col.isActive ? 'Active' : 'Inactive'} />
                  </td>
                  <td className="px-3">
                    <div className="flex items-center gap-1 justify-end">
                      <button onClick={() => openEditor(col)} title="Edit" className="p-1.5 text-[var(--admin-text-mute)] hover:text-[var(--admin-text)] rounded focus:outline-none focus:ring-2 focus:ring-[var(--admin-focus)]">
                        <Pencil className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => setDeleteTarget(col)} title="Delete" className="p-1.5 text-[var(--admin-text-mute)] hover:text-[var(--admin-danger)] rounded focus:outline-none focus:ring-2 focus:ring-[var(--admin-focus)]">
      ...
      {/* Slide-over editor */}
      {editing !== null && (
        <div className="fixed inset-0 z-40 flex" role="dialog" aria-modal="true" aria-label="Edit Collection">
          ...
          <div className="relative ml-auto z-10 w-full max-w-lg bg-[var(--admin-panel)] h-full shadow-xl flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--admin-border)] flex-shrink-0">
              <h2 className="font-semibold text-base">{editing === 'new' ? 'Add Collection' : 'Edit Collection'}</h2>
              <button onClick={() => setEditing(null)} className="text-[var(--admin-text-mute)] hover:text-[var(--admin-text)] focus:outline-none focus:ring-2 focus:ring-[var(--admin-focus)] rounded p-1">✕</button>
            </div>
            ...
            <div className="flex border-b border-[var(--admin-border)] px-6 flex-shrink-0">
              {(['details', 'products'] as EditorTab[]).map(tab => (
                ...
                >
                  {tab}
                  {tab === 'products' && editing !== 'new' && <span className="ml-1.5 text-xs text-[var(--admin-text-faint)]">({members.length})</span>}
                </button>
              ))}
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {activeTab === 'details' && (
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                  ...
                  <input type="hidden" name="isFeatured" value={editing !== 'new' ? String(editing.isFeatured) : 'false'} />

                  <div>
                    <label className="block text-xs text-[var(--admin-text-mute)] mb-1" htmlFor="col-name">Name *</label>
                    <input id="col-name" name="name" required defaultValue={editing !== 'new' ? editing.name : ''}
                  ...
                  <div>
                    <label className="block text-xs text-[var(--admin-text-mute)] mb-1" htmlFor="col-slug">Slug</label>
                    <input id="col-slug" name="slug" placeholder="auto-generated" defaultValue={editing !== 'new' ? editing.slug : ''}
                      ...
                      onChange={e => { e.target.value = slugify(e.target.value); }} />
                  </div>

                  <div>
                    <label className="block text-xs text-[var(--admin-text-mute)] mb-1" htmlFor="col-desc">Description</label>
                    <textarea id="col-desc" name="description" rows={2} defaultValue={editing !== 'new' ? (editing.description ?? '') : ''}
                  ...
                  <div className="flex gap-4">
                    <label className="flex items-center gap-2 text-sm cursor-pointer">
                      <input type="checkbox" name="isFeatured" value="true" defaultChecked={editing !== 'new' && editing.isFeatured}
                        className="rounded focus:ring-[var(--admin-focus)]" />
                      Featured
                    </label>
                    <label className="flex items-center gap-2 text-sm cursor-pointer">
                      <input type="checkbox" name="isActive" value="true" defaultChecked={editing === 'new' || editing.isActive}
                        className="rounded focus:ring-[var(--admin-focus)]" />
                      Active
                    </label>
                  </div>
                  ...
                  <div>
                    <label className="block text-xs text-[var(--admin-text-mute)] mb-1" htmlFor="col-sort">Sort Order</label>
                    <input id="col-sort" name="sortOrder" type="number" min={0} defaultValue={editing !== 'new' ? editing.sortOrder : 0}
                      className="w-full px-3 py-2 text-sm border border-[var(--admin-border)] rounded-[var(--admin-radius)] bg-[var(--admin-bg)] focus:outline-none focus:ring-2 focus:ring-[var(--admin-focus)]" />
                  </div>

                  <div>
                    <label className="block text-xs text-[var(--admin-text-mute)] mb-1" htmlFor="col-meta-title">Meta Title (≤60)</label>
                    <input id="col-meta-title" name="metaTitle" maxLength={60} defaultValue={editing !== 'new' ? (editing.metaTitle ?? '') : ''}
                      className="w-full px-3 py-2 text-sm border border-[var(--admin-border)] rounded-[var(--admin-radius)] bg-[var(--admin-bg)] focus:outline-none focus:ring-2 focus:ring-[var(--admin-focus)]" />
                  </div>

                  <div>
                    <label className="block text-xs text-[var(--admin-text-mute)] mb-1" htmlFor="col-meta-desc">Meta Description (≤160)</label>
                    <textarea id="col-meta-desc" name="metaDescription" maxLength={160} rows={2} defaultValue={editing !== 'new' ? (editing.metaDescription ?? '') : ''}
                      className="w-full px-3 py-2 text-sm border border-[var(--admin-border)] rounded-[var(--admin-radius)] bg-[var(--admin-bg)] resize-none focus:outline-none focus:ring-2 focus:ring-[var(--admin-focus)]" />
                  </div>
              ...
              {activeTab === 'products' && editing !== 'new' && (
                <div className="p-6 space-y-4">
                  ...
                  {searchResults.length > 0 && (
                    <div className="border border-[var(--admin-border)] rounded-[var(--admin-radius)] overflow-hidden">
                      {searchResults.map(p => {
                        ...
                        return (
                          <div key={p.id} className="flex items-center gap-3 px-3 py-2 hover:bg-[var(--admin-surface)] border-b border-[var(--admin-border)] last:border-0">
                            {p.images[0] && <img src={p.images[0].src} alt="" className="w-8 h-8 rounded object-cover flex-shrink-0" />}
                  ...
                  {/* Members */}
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-[var(--admin-text-faint)] mb-2">In this collection ({members.length})</p>
                    {members.length === 0
                      ? <p className="text-sm text-[var(--admin-text-mute)]">No products yet. Search above to add some.</p>
                      : (
                        <div className="space-y-1.5">
                          {members.map(m => (
                            <div key={m.productId} className="flex items-center gap-3 px-3 py-2 border border-[var(--admin-border)] rounded-[var(--admin-radius)]">
                              {m.product.images[0] && <img src={m.product.images[0].src} alt="" className="w-8 h-8 rounded object-cover flex-shrink-0" />}
                              <span className="flex-1 text-sm">{m.product.name}</span>
                              <button onClick={() => handleRemoveProduct(m.productId)} className="p-1 text-[var(--admin-text-mute)] hover:text-[var(--admin-danger)] focus:outline-none" aria-label="Remove">
