    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-[var(--admin-text-mute)]">
          {categories.length} top-level, {categories.reduce((s, c) => s + c.children.length, 0)}{' '}
          subcategories
        </p>
        <button
          onClick={() => openNew()}
        ...
        <div className="bg-[var(--admin-panel)] border border-[var(--admin-border)] rounded-lg overflow-hidden">
          {topLevelCategories.map((cat, idx) => (
            <div
              key={cat.id}
              className={
                idx < topLevelCategories.length - 1 ? 'border-b border-[var(--admin-border)]' : ''
              }
            >
              {/* Parent row */}
              <div className="flex items-center gap-2 px-3 h-[var(--admin-row-h)] hover:bg-[var(--admin-surface)] group">
                ...
                <span className="flex-1 font-medium text-sm text-[var(--admin-text)]">
                  {cat.name}
                </span>
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button
                    ...
                    className="p-1.5 text-[var(--admin-text-mute)] rounded focus:outline-none focus:ring-2 focus:ring-[var(--admin-focus)]"
                  >
                    {cat.isActive ? (
              ...
              {expanded.has(cat.id) &&
                cat.children.map((child) => (
                  <div
                    key={child.id}
                    className="flex items-center gap-2 px-3 pl-10 h-[var(--admin-row-h)] border-t border-[var(--admin-border)] bg-[var(--admin-surface)]/50 hover:bg-[var(--admin-surface)] group"
                  >
                    <span className="flex-1 text-sm text-[var(--admin-text)]">{child.name}</span>
                    <span className="text-xs text-[var(--admin-text-faint)] font-mono">
                      {child.slug}
                    </span>
                    <a
                    ...
                    <StatusBadge
                      status={child.isActive ? 'ACTIVE' : 'ARCHIVED'}
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  {categories
                    .filter(
                      (c) => c.parentId === null && (editing === 'new' || editing?.id !== c.id)
                    )
                    .map((c) => (
      ...
      <ConfirmDialog
        ...
        description={
          deleteTarget
            ? `Delete "${deleteTarget.name}"? Products in this category will not be deleted, but will lose their category assignment.`
            : ''
        }
        confirmLabel="Delete"
        destructive
