...
type ContactInboxProps = {
  ...
  counts: { all: number; new: number; resolved: number };
};

export function ContactInboxClient({ data, meta, counts }: ContactInboxProps) {
                ...
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider ${statusColor[sub.status] || 'bg-gray-100 text-gray-600'}`}
                      >
                        {sub.status}
                      </span>
                    </div>
                ...
                {isOpen && (
                  <div className="border-t p-5 bg-gray-50 space-y-4">
                    ...
                    <div>
                      ...
                      <button
                        onClick={() =>
                          handleSaveNotes(sub.id, notes[sub.id] ?? sub.internalNotes ?? '')
                        }
                    ...
                    <div className="flex gap-2">
                      {sub.status !== 'in_progress' && (
                        <button
                          onClick={() => handleStatus(sub.id, 'in_progress')}
