...
import { Search, Upload, Filter, Image as ImageIcon, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function MediaGalleryClient({ assets, meta, folders, mimeTypes, currentSort }: Props) {
  ...
  const handleUpload = async (files: FileList | null) => {
    ...
    // Add to UI state
    setUploads((prev) => [...newUploads.map(u => ({ id: u.id, name: u.name, progress: 0 })), ...prev]);

    // Process each
    for (const u of newUploads) {
      if (u.file.size > 8 * 1024 * 1024) {
        setUploads((prev) => prev.map(p => p.id === u.id ? { ...p, progress: 100, error: 'Exceeds 8MB limit' } : p));
        continue;
      }
      ...
      try {
        ...
        await upload(u.name, u.file, {
          ...
          onUploadProgress: (event) => {
            setUploads((prev) => prev.map(p => p.id === u.id ? { ...p, progress: event.percentage } : p));
          },
        });
        ...
        setUploads((prev) => prev.map(p => p.id === u.id ? { ...p, error: error.message } : p));
      }
    }
  };
    ...
    >
      ...
      {uploads.length > 0 && (
        <div className="mb-6 space-y-2">
          {uploads.map(u => (
            <div key={u.id} className="flex items-center gap-4 bg-[var(--admin-surface)] p-3 rounded-[var(--admin-radius)] border border-[var(--admin-border)]">
      ...
      {assets.length === 0 ? (
        <EmptyState 
          ...
          description={searchParams.toString() ? "Try adjusting your filters or search term." : "Upload your first image to start building your media library."}
          action={
            <button onClick={() => fileInputRef.current?.click()} className="px-3 py-2 text-sm font-medium rounded-[var(--admin-radius)] bg-[var(--admin-accent)] text-white">
              Upload Image
            </button>
          }
        />
            ...
            {assets.map(asset => (
              ...
              >
                ...
                <div className="w-full p-2 border-t border-[var(--admin-border)]">
                  <p className="text-xs font-medium text-[var(--admin-text)] truncate">{asset.filename}</p>
                  <div className="flex justify-between items-center mt-1">
                    <p className="text-[10px] text-[var(--admin-text-faint)] font-mono">{(asset.sizeBytes / 1024).toFixed(0)} KB</p>
                    <p className="text-[10px] text-[var(--admin-text-faint)] font-mono">{asset.width}×{asset.height}</p>
                  </div>
                </div>
              </button>
            ))}
