    ...
    <div className="p-4 md:p-8 max-w-[1440px] mx-auto pb-32">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div className="flex items-center gap-4">
          <Link href="/admin/orders" className="text-gray-500 hover:text-black">← Back</Link>
          <h1 className="text-2xl font-[var(--font-jost)] font-semibold">Order {order.orderNumber}</h1>
          <StatusBadge status={order.status} />
        </div>
        <div className="flex items-center gap-2">
          <a href={`/admin/orders/${order.id}/print`} target="_blank" rel="noopener noreferrer" className="px-4 py-2 border rounded hover:bg-gray-50 text-sm">Print Packing Slip</a>
          {canCancel && (
            <ConfirmDialog 
              trigger={<button className="px-4 py-2 border border-red-200 text-red-600 rounded hover:bg-red-50 text-sm">Cancel Order</button>}
              title="Cancel Order"
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-1/3 flex flex-col gap-6 order-1 md:order-2">
            ...
            {order.status === 'shipped' || order.status === 'delivered' ? (
              <div className="space-y-3">
                <div className="text-sm">
                  ...
                  <span className="font-medium">{order.status === 'shipped' ? 'Shipped' : 'Delivered'}</span>
                </div>
                {order.carrier && (
                  <div className="text-sm">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <input type="text" value={carrier} onChange={e => setCarrier(e.target.value)} className="w-full p-2 border rounded text-sm" placeholder="e.g. DHL, FedEx" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-700">Tracking Number (optional)</label>
                  <input type="text" value={trackingNumber} onChange={e => setTrackingNumber(e.target.value)} className="w-full p-2 border rounded text-sm" />
                </div>
                <button 
                  onClick={handleMarkShipped} 
                  disabled={isProcessing || (order.status !== 'confirmed' && order.status !== 'new')}
          ...
          <div className="bg-white border rounded shadow-sm p-5">
            ...
            <div className="space-y-1">
              <div className="font-medium">
                {order.customerId ? (
                  <Link href={`/admin/customers/${order.customerId}`} className="text-[var(--admin-accent)] hover:underline">
                    {order.customerFirstName} {order.customerLastName}
                  </Link>
                ) : (
                  <span>{order.customerFirstName} {order.customerLastName} (Guest)</span>
                )}
              </div>
          ...
          <div className="bg-white border rounded shadow-sm p-5 relative">
            <button onClick={handleCopyAddress} className="absolute top-4 right-4 text-xs text-blue-600 hover:underline">Copy</button>
              ...
              {order.items.map((item: any) => (
                <div key={item.id} className="p-5 flex items-start gap-4">
                  {item.imageUrlSnapshot ? (
                    <Image src={item.imageUrlSnapshot} alt={item.productNameSnapshot} width={60} height={80} className="object-cover rounded bg-gray-100" />
                  ) : (
                    <div className="w-[60px] h-[80px] bg-gray-100 rounded flex items-center justify-center text-xs text-gray-400">No Img</div>
                  )}
                  <div className="flex-1">
                    <div className="flex justify-between">
                      {item.productId ? (
                        <Link href={`/admin/products/${item.productId}`} className="font-medium text-[var(--admin-accent)] hover:underline">
                          {item.productNameSnapshot}
                        </Link>
                      ) : (
                        <span className="font-medium">{item.productNameSnapshot}</span>
                      )}
                      <span className="font-medium">{formatMoney(item.unitPrice * item.quantity, order.currency)}</span>
                    </div>
                    <div className="text-sm text-gray-600 mt-1">
                      {item.productSkuSnapshot && <span className="mr-3">SKU: {item.productSkuSnapshot}</span>}
                      {item.sizeCodeSnapshot && <span className="mr-3">Size: {item.sizeCodeSnapshot}</span>}
              ...
              <div className="relative border-l-2 border-gray-200 ml-3 space-y-6">
                {order.statusEvents.map((event: any) => (
                  <div key={event.id} className="relative pl-6">
                    ...
                    <div className="text-sm font-medium">Status changed to {event.toStatus.toUpperCase()}</div>
                    <div className="text-xs text-gray-500 mt-1" title={new Date(event.createdAt).toLocaleString()}>
                      {formatDistanceToNow(new Date(event.createdAt), { addSuffix: true })} by {event.actorId || 'System'}
                    </div>
                    {event.note && (
