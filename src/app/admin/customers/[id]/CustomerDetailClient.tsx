...
export function CustomerDetailClient({ customer, stats, isOwner }: CustomerDetailProps) {
  ...
  const handleAnonymise = async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to anonymise customer');
    } finally {
  ...
  const isAnonymised =
    customer.firstName === 'Deleted' && customer.email.includes('anonymised.local');

  return (
    <div className="p-4 md:p-8 max-w-[1440px] mx-auto pb-32">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div className="flex items-center gap-4">
          <Link href="/admin/customers" className="text-gray-500 hover:text-black">
            ← Back
          </Link>
          ...
          {isAnonymised && (
            <span className="px-2 py-1 bg-gray-200 text-gray-800 text-xs font-medium rounded-full uppercase tracking-wider">
              Anonymised
            </span>
          )}
        </div>
          ...
          <div className="bg-white border rounded shadow-sm p-5">
            ...
            <div className="space-y-3 text-sm">
              <div>
                ...
                {isAnonymised ? (
                  <span className="italic text-gray-400">{customer.email}</span>
                ) : (
              ...
              <div className="space-y-4">
                {customer.addresses.map((address) => (
                  <div key={address.id} className="text-sm border rounded p-3 bg-gray-50">
                    {address.isDefault && (
                      <div className="text-xs font-semibold text-[var(--admin-accent)] mb-1">
                        Default
                      </div>
                    )}
          ...
          <div className="bg-white border rounded shadow-sm p-5">
            ...
            <div className="text-sm text-gray-500">
              ...
              {customer.wishlistItems
                ? `${customer.wishlistItems.length} items in wishlist`
                : '0 items'}
            </div>
          </div>
        </div>
