      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white border rounded-lg p-5 shadow-sm">
          ...
          <div className="text-xs text-gray-500 mt-1">
            ...
            {yesterdaySales > 0 && (
              <span
                className={`ml-2 font-medium ${pctChange >= 0 ? 'text-green-600' : 'text-red-500'}`}
              >
                {pctChange >= 0 ? '+' : ''}
        ...
        <div className="bg-white border rounded-lg p-5 shadow-sm">
          ...
          <Link
            ...
            className="text-xs text-[var(--admin-accent)] hover:underline mt-1 block"
          >
            View new orders →
          </Link>
        </div>
        ...
        <div className="bg-white border rounded-lg p-5 shadow-sm">
          ...
          <div className="text-2xl font-bold">
            {formatMoney(last30Revenue._sum.total || 0, 'NPR')}
          </div>
          ...
          <table className="w-full text-sm">
            <tbody className="divide-y">
              {recentOrders.length === 0 ? (
                <tr>
                  <td className="py-12 text-center text-gray-400">No orders yet.</td>
                </tr>
              ) : (
                recentOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50">
                    ...
                    <td className="py-3 px-5 font-medium text-right">
                      {formatMoney(order.total, order.currency)}
                    </td>
                    <td className="py-3 px-5 text-right">
                      <span
                        className={`px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider ${STATUS_COLORS[order.status] || 'bg-gray-100 text-gray-600'}`}
                      >
                        {order.status}
                      </span>
                    </td>
                  ...
                  <div key={s.id} className="px-5 py-3 flex items-center justify-between gap-2">
                    <div>
                      <Link
                        ...
                        className="text-sm font-medium hover:underline text-[var(--admin-accent)]"
                      >
                        {s.product.name}
                      </Link>
                    ...
                    <span
                      className={`text-sm font-bold ${s.stockQuantity === 0 ? 'text-red-600' : 'text-amber-600'}`}
                    >
                      {s.stockQuantity}
                    </span>
                  </div>
