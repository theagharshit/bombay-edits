...
const HERO_FIELDS = [
  {
    ...
    label: 'Hero Title',
    type: 'text',
    placeholder: 'Exquisite Indian Wear',
  },
  {
    key: 'homepage_hero_subtitle',
    label: 'Hero Subtitle',
    type: 'text',
    placeholder: 'Handcrafted for the modern woman',
  },
  {
  ...
  },
  {
    ...
    type: 'text',
    placeholder: 'Free shipping over Rs. 5,000 · WhatsApp us',
  },
];
...
export function HomepageClient({ settings, collections }: HomepageClientProps) {
  const router = useRouter();
  const [form, setForm] = useState<Record<string, string>>(
    Object.fromEntries(HERO_FIELDS.map((f) => [f.key, String(settings[f.key] ?? '')]))
  );
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await upsertManyStoreSettings({
        ...form,
        homepage_featured_collection_ids: featuredIds,
      });
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Configure what appears on the storefront homepage.
          </p>
        </div>
        <div className="flex gap-2">
          <a
            ...
            className="flex items-center gap-1.5 px-3 py-2 border rounded text-sm hover:bg-gray-50"
          >
      ...
      <div className="bg-white border rounded-lg shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b bg-gray-50">
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Select which collections appear in the homepage grid.
          </p>
        </div>
        <div className="p-5">
          {collections.length === 0 ? (
            <p className="text-sm text-gray-400">
              ...
              <a href="/admin/collections" className="text-[var(--admin-accent)] hover:underline">
                Create one
              </a>
              .
            </p>
