...
export function ProductCard({ product, priority = false }: ProductCardProps) {
  ...
  const frontImage =
    product.images.find((img) => img.type === 'front')?.src ||
    getProductPlaceholder('front', product.slug);
  const backImage =
    product.images.find((img) => img.type === 'back' || img.type === 'lifestyle')?.src ||
  ...
  return (
    <div className="group relative">
      ...
      <Link
        ...
        className="block product-image-hover aspect-[3/4] bg-cream relative overflow-hidden"
      >
        <Image
          src={frontImage}
          alt={product.name}
      ...
      <button
        ...
        className={`absolute top-3 right-3 p-2 rounded-full transition-all ${
          wishlisted
            ? 'bg-ivory text-wine'
            : 'bg-ivory/70 text-deep-brown opacity-0 group-hover:opacity-100'
        }`}
      ...
      <div className="mt-4 space-y-1">
        <Link href={`/shop/${product.slug}`} className="block">
          <h3 className="text-xs uppercase tracking-widest font-medium text-ink truncate mb-1">
            {product.name}
          </h3>
        </Link>
